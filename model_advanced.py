...
class AdvancedPredictionModel:
    """
    Modelo estadístico basado en Poisson independientes para proyección de mercados.
    """
    ...
    def _poisson_goal_probability(self, lambda_a: float, lambda_b: float, 
        ...
        """
        Calcula probabilidades de resultado usando dos Poisson independientes.
        ...
        if lambda_a < 0 or lambda_b < 0:
            raise ValueError("Las expectativas de gol no pueden ser negativas")
        adaptive_max = int(np.ceil(poisson.ppf(0.99999, max(lambda_a, lambda_b))))
        max_goals = max(max_goals, adaptive_max)
        prob_home_win = 0.0
        prob_draw = 0.0
        prob_away_win = 0.0
        ...
        if total <= 0:
            raise RuntimeError("No se pudo construir la distribución de marcador")
        return {"home_win": prob_home_win / total, "draw": prob_draw / total,
                "away_win": prob_away_win / total}
    ...
    def _calculate_confidence_interval(self, mean: float, variance: float, 
        ...
        if not np.isfinite(mean) or not np.isfinite(variance) or variance < 0:
            raise ValueError("Media y varianza deben ser finitas, con varianza no negativa")
        if sample_size < 2:
    ...
    def predict_market(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Un diferencial positivo implica más fatiga local: reduce su lambda.
        ...
        lambda_a = max(0.3, base_lambda_a + form_impact / 2 + home_impact + h2h_impact / 2 - fatigue_impact / 2)
        lambda_b = max(0.3, base_lambda_b - form_impact / 2 - h2h_impact / 2 + fatigue_impact / 2)
        ...
        under_2_5_probability = poisson.cdf(2, proj_total_goals)
        ...
        return {
            "goals": {
                ...
                "total_goals_probabilities": {
                    "under_2_5": round(under_2_5_probability * 100, 1),
                    "over_2_5": round((1 - under_2_5_probability) * 100, 1)
                },
                "adjustments_applied": {
    ...
    def generate_betting_recommendations(self, predictions: Dict[str, Any], 
        ...
        if total_goals < 2.2 and goals_data.get('confidence_level', 0) > 65:
            ...
            if odds_data and 'under_2_5' in odds_data:
                probability = goals_data.get("total_goals_probabilities", {}).get("under_2_5", 0) / 100
                ev = self.calculate_expected_value(probability, odds_data['under_2_5'])
            ...
            if odds_data and 'over_2_5' in odds_data:
                probability = goals_data.get("total_goals_probabilities", {}).get("over_2_5", 0) / 100
                ev = self.calculate_expected_value(probability, odds_data['over_2_5'])
                ev_info = f" | EV: {ev*100:+.1f}%" if ev > 0.05 else " | Sin valor"
