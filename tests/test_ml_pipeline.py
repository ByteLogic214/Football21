...
def test_matriz_ignora_estadisticas_del_partido_actual(team_frame):
    """Alterar el resultado/estadísticas actuales no debe cambiar X."""
    original, _, _ = build_model_matrix(team_frame)
    altered = team_frame.copy()
    altered["goals_for"] = 99.0
    altered["goals_against"] = 0.0
    altered["xg_for"] = 99.0
    altered["total_shots_for"] = 999.0
    candidate, _, _ = build_model_matrix(altered)
...
def test_timeseries_no_separa_partidos_de_la_misma_fecha(model_matrix):
    """Una jornada no puede aparecer parcialmente en entrenamiento y test."""
    X, y, meta = model_matrix
    model = MatchOutcomeModel(n_splits=3, backend="lightgbm")
    Xs, ys, ds = model.sort_by_date(X, y, meta["date"])
    for tr, te in model.time_splits(Xs, ys, ds):
        assert set(ds.iloc[tr]).isdisjoint(set(ds.iloc[te]))
