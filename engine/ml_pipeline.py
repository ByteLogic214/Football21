...
class MatchOutcomeModel:
    ...
    def time_splits(self, X: pd.DataFrame, y: np.ndarray,
        ...
        """Walk-forward split by match date, never inside a matchday.

        ``TimeSeriesSplit`` over rows can put matches from the same date in
        both train and validation.  That leaks same-round information through
        standings/form features.  Splitting the ordered unique dates first
        makes the temporal boundary strict even when a round has many games.
        """
        d = pd.to_datetime(dates, utc=True)
        if not d.is_monotonic_increasing:
            raise ValueError("Los datos deben estar ordenados por fecha")
        unique_dates = pd.Index(d.drop_duplicates())
        if len(unique_dates) <= self.n_splits:
            raise ValueError(
                "No hay suficientes fechas distintas para validación temporal "
                f"(fechas={len(unique_dates)}, folds={self.n_splits})")
        splitter = TimeSeriesSplit(n_splits=self.n_splits)
        for train_dates, test_dates in splitter.split(unique_dates):
            train_idx = np.flatnonzero(d.isin(unique_dates[train_dates]))
            test_idx = np.flatnonzero(d.isin(unique_dates[test_dates]))
            if d.iloc[train_idx].max() >= d.iloc[test_idx].min():
                raise RuntimeError("Split temporal no estricto")
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        ...
        raw = self.model_.predict_proba(X_in)
        # Some early walk-forward folds can lack a rare outcome.  Align the
        # estimator's learned classes to the public [home, draw, away]
        # contract instead of silently assigning a wrong column to a class.
        learned_classes = np.asarray(getattr(self.model_, "classes_", CLASSES))
        if np.array_equal(learned_classes, CLASSES):
            return validate_probabilities(raw)
        aligned = np.full((len(raw), len(CLASSES)), 1e-12, dtype=float)
        for source, label in enumerate(learned_classes):
            aligned[:, int(label)] = raw[:, source]
        aligned /= aligned.sum(axis=1, keepdims=True)
        return validate_probabilities(aligned)
