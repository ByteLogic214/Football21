...
# Only information known before kick-off can reach the outcome model.  Raw
# match statistics (goals, xG, shots, etc.) are retained in the team frame so
# they can create *future* rolling features and the training target, but must
# never be used as features for that same match.
_ROLLING_SOURCE_FEATURES = (
    "goals_for", "goals_against", "points", "xg_for", "xg_against",
    "total_shots_for", "total_shots_against", "shots_on_target_for",
    "shots_on_target_against", "key_passes_for", "key_passes_against",
    "conversion_rate", "shot_accuracy",
)
MODEL_FEATURES = [f"roll_{feature}" for feature in _ROLLING_SOURCE_FEATURES] + ["rest_days"]
...
def add_rolling_features(frame: pd.DataFrame, window: int = 5, min_periods: int = 3, expanding_min_periods: int = 8) -> pd.DataFrame:
    ...
    available = [column for column in _ROLLING_SOURCE_FEATURES if column in out.columns]
