...
def test_todas_las_features_de_modelo_son_previas_al_partido():
    """El modelo no puede recibir el marcador ni las stats del mismo partido."""
    from engine.thestats_features import MODEL_FEATURES
    assert "goals_for" not in MODEL_FEATURES
    assert "xg_for" not in MODEL_FEATURES
    assert all(feature.startswith("roll_") or feature == "rest_days"
               for feature in MODEL_FEATURES)


def test_rolling_generates_prematch_statistical_features():
    from engine.thestats_features import add_rolling_features
    df = pd.DataFrame([
        {"match_id": "m1", "team_id": "t", "date": pd.Timestamp("2025-01-01", tz="UTC"),
         "goals_for": 1.0, "goals_against": 0.0, "points": 3.0, "xg_for": 1.1,
         "xg_against": 0.5, "total_shots_for": 10.0, "total_shots_against": 4.0,
         "shots_on_target_for": 4.0, "shots_on_target_against": 1.0,
         "key_passes_for": 3.0, "key_passes_against": 1.0,
         "conversion_rate": 0.1, "shot_accuracy": 0.4},
        {"match_id": "m2", "team_id": "t", "date": pd.Timestamp("2025-01-08", tz="UTC"),
         "goals_for": 0.0, "goals_against": 1.0, "points": 0.0, "xg_for": 0.6,
         "xg_against": 1.2, "total_shots_for": 6.0, "total_shots_against": 12.0,
         "shots_on_target_for": 2.0, "shots_on_target_against": 5.0,
         "key_passes_for": 1.0, "key_passes_against": 4.0,
         "conversion_rate": 0.0, "shot_accuracy": 1 / 3},
    ])
    out = add_rolling_features(df, min_periods=1)
    assert out.loc[out["match_id"] == "m2", "roll_xg_against"].item() == 0.5
    assert out.loc[out["match_id"] == "m2", "roll_total_shots_for"].item() == 10.0
